...
{if $displayedFacets|count}
  <div id="search_filters">
    ...
    {foreach from=$displayedFacets item="facet"}
      <section class="facet clearfix facets__{$facet.label|lower}">
        ...
        <div class="title hidden-md-up" data-target="#facet_{$_expand_id}" data-toggle="collapse"{if !$_collapse} aria-expanded="true"{/if}>
          <p class="h6 facet-title facets__{$filter.label|lower}">{$facet.label}</p>
          ...
          {block name='facet_item_other'}
            <ul id="facet_{$_expand_id}" class="collapse{if !$_collapse} in{/if}">
              {foreach from=$facet.filters key=filter_key item="filter"}
                ...
                <li class="filter_item facets__{$facet.label|lower}-item__{$filter.label|lower}">
